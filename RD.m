...
% When RD=[500,1000,1500,2000],reverse solution of Laplace space bottomhole pressure (equation (62)) to real space bottomhole pressure (equation (63)) 
for k=1:4
    for i=1:L
        ft(i,k)=0;
        u=log(2)/tD(i);
        for j=1:M
           ...
           % Code for guide function: equation (22) 
            ...
            E=((r1*RD(k))^(1-(df2-Theta2)/2))*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S));
           
            % Code for guide function: equation (25) 
            F=(r1^(1-(df2-Theta2)/2))*(RD(k)^(-(df2-Theta2)/2))*((2+Theta2-df2)*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))+sqrt(sigma*z/CDE2S)...
                *(RD(k)^(1+Theta2/2))*posimn(v2,v2+1,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S)));
          
           % Code for guide function: equation (26) 
            G=(r1^(-(df2-Theta2)/2))*(RD(k)^(1-(df2-Theta2)/2))*((2+Theta2-df2)*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))-sqrt(sigma*z/CDE2S)...
                *(r1^(1+Theta2/2))*posimn(v2+1,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S)));
           
           % Code for guide function: equation (27) 
            H=((r1*RD(k))^(-(df2-Theta2)/2))*(((2+Theta2-df2)^2)*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))-sqrt(sigma*z/CDE2S)*(2+Theta2-df2)...
                *(r1^(1+Theta2/2))*posimn(v2+1,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))+sqrt(sigma*z/CDE2S)*(2+Theta2-df2)*(RD(k)^(1+Theta2/2))...
                *posimn(v2,v2+1,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))-(sigma*z/CDE2S)*((r1*RD(k))^(1+Theta2/2))*posimn(v2+1,v2+1,r1^(1+Theta2/2),...
                RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S)));
            
           
             % Code for guide function: equation (28) and when m=1 (or equation (60)) 
            P=((r1*RD(k))^(1-(df2-Theta2)/2))*((v2/(2*z))*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))+(1/(2*sqrt(z)))*(2/(Theta2+2))*sqrt(sigma/CDE2S)...
                *(RD(k)^(1+Theta2/2))*posimn(v2,v2+1,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S)));
           
             % Code for guide function: equation (29) and when m=1 (or equation (61)) 
            Q=(r1^(-(df2-Theta2)/2))*(RD(k)^(1-(df2-Theta2)/2))*((v2/(2*z))*((2+Theta2-df2)*posimn(v2,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))-(r1^(1+Theta2/2))...
                *sqrt(sigma*z/CDE2S)*posimn(v2+1,v2,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S)))+(2/(Theta2+2))*sqrt(sigma/CDE2S)*(RD(k)^(1+Theta2))*(1/(2*sqrt(z)))...
                *((2+Theta2-df2)*posimn(v2,v2+1,r1^(1+Theta2/2),RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))-(r1^(1+Theta2))*sqrt(sigma*z/CDE2S)*posimn(v2+1,v2+1,r1^(1+Theta2/2),...
                RD(k)^(1+Theta2/2),(2/(Theta2+2))*sqrt(sigma*z/CDE2S))));
            
            % Code for numerator of outer regions' kernel functions equation (equation(58))
            fz2=(Alpha3*(RD(k)^2)+Alpha4*RD(k)+c)*E-(Alpha1*RD(k)+Alpha2)*P-RD(k)*F;
            
            % Code for denominator of outer regions' kernel functions equation (equation(58)) 
            fm2=(Alpha3*(RD(k)^2)+Alpha4*RD(k)+c)*G-(Alpha1*RD(k)+Alpha2)*Q-RD(k)*H;
...
% Three point method for derivative calculation (pressure derivative in thesense of well testing analysis),when RD=[500,1000,1500,2000]
for k=1:4
...
% Double logarithmic curves of bottomhole pressure at different RD
...
% Double logarithmic curves of bottomhole pressure derivatives at different RD
...
h=legend('$$R_{D}=500$$','$$R_{D}=1000$$','$$R_{D}=1500$$','$$R_{D}=2000$$');
