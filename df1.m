...
% When df1=[1.4,1.5,1.6,1.7],reverse solution of Laplace space bottomhole pressure (equation (62)) to real space bottomhole pressure (equation (63)) 
for k=1:4
    for i=1:L
        ft(i,k)=0;
        u=log(2)/tD(i);
        for j=1:M
            ...
            A=(r1^(1-(df1(k)-Theta1)/2))*posimn(v1,v1,1,r1^((Theta1+2)/2),(2/(Theta1+2))*sqrt(z/CDE2S));
           
            % Code for guide function: equation (21) 
            B=(r1^(-(df1(k)-Theta1)/2))*((2+Theta1-df1(k))*posimn(v1,v1,1,r1^((Theta1+2)/2),(2/(Theta1+2))*sqrt(z/CDE2S))+sqrt(z/CDE2S)*(r1^((2+Theta1)/2))*posimn(v1,v1+1,1,r1^((Theta1+2)/2),...
                (2/(Theta1+2))*sqrt(z/CDE2S)));
           
            % Code for guide function: equation (22)
            C=(r1^(1-(df1(k)-Theta1)/2))*((2+Theta1-df1(k))*posimn(v1,v1,1,r1^((Theta1+2)/2),(2/(Theta1+2))*sqrt(z/CDE2S))-sqrt(z/CDE2S)*(1^((2+Theta1)/2))*posimn(v1+1,v1,1,r1^((Theta1+2)/2),...
                (2/(Theta1+2))*sqrt(z/CDE2S)));
          
            % Code for guide function: equation (23)
            D=(r1^(-(df1(k)-Theta1)/2))*(((2+Theta1-df1(k))^2)*posimn(v1,v1,1,r1^((Theta1+2)/2),(2/(Theta1+2))*sqrt(z/CDE2S))-sqrt(z/CDE2S)*(2+Theta1-df1(k))*posimn(v1+1,v1,1,r1^((Theta1+2)/2),...
                (2/(Theta1+2))*sqrt(z/CDE2S))+sqrt(z/CDE2S)*(2+Theta1-df1(k))*(r1^(1+Theta1/2))*posimn(v1,v1+1,1,r1^((Theta1+2)/2),(2/(Theta1+2))*sqrt(z/CDE2S))-(z/CDE2S)*(r1^(1+Theta1/2))...
           ...
           % Code for guide function: equation (24)
            ...
            fz1=Lambda*r1^(df2-df1(k)-(Theta2-Theta1))*A-fei2*B;
           
            % Code for denominator of inner  regions' kernel functions equation (equation(59))
            fm1=Lambda*r1^(df2-df1(k)-(Theta2-Theta1))*C-fei2*D;
...
% Three point method for derivative calculation (pressure derivative in thesense of well testing analysis),when df1=[1.4,1.5,1.6,1.7]
for k=1:4
...
% Double logarithmic curves of bottomhole pressure at different df1
...
% Double logarithmic curves of bottomhole pressure derivatives at different df1
...
h=legend('$$d_{f1}=1.4$$','$$d_{f1}=1.5$$','$$d_{f1}=1.6$$','$$d_{f1}=1.7$$');
